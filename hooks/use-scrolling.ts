...
export function useScrolling(target?: ScrollTarget, options: UseScrollingOptions = {}): boolean {
  ...
  useEffect(() => {
    ...
    const eventTarget: EventTargetWithScroll =
      fallbackToDocument && element === window && typeof document !== 'undefined' ? document : element;

    const on = (el: EventTargetWithScroll, event: string, handler: EventListener) => el.addEventListener(event, handler, true);

    const off = (el: EventTargetWithScroll, event: string, handler: EventListener) => el.removeEventListener(event, handler);
